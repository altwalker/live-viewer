...
def server(tests, models, executor, url, port, graphwalker_port, delay):
    ...
    click.secho("Starting server...", fg='green', bold=True)
...
@cli.command()
def open():
    """Starts a web browser with the view."""

    path = pathlib.Path(__file__).parent
    view_path = pathlib.Path("../ui/index.html")

    click.secho("Open webbrowser...", fg='green', bold=True)
    webbrowser.open(path.joinpath(view_path).resolve().as_uri())
